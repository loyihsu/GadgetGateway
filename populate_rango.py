...
def populate():
    ...
    for category, data in categories.items():
        ...
        for product in data:
            add_product(product['name'], product['description'], product['image'], cat, product['views'])
...
def add_product(name, description, image, category, views=0):
    temp = Product.objects.get_or_create(name=name, category=category)[0]
    temp.description = description
    temp.views = views
    temp.image = 'product_image/' + image
    temp.save()
    return temp
